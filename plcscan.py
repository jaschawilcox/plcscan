...
def scan(argv):
    ...
    if options.hosts_file:
        try:
            with open(options.hosts_file, 'r') as f:
                scan_hosts = [line.strip() for line in f]
        except IOError:
            print("Can't open file {}".format(options.hosts_file))
    ...
    for host in scan_hosts:
        ...
        for port in ports:
            ...
            if not res:
                print("{}:{} unknown protocol".format(host, port))
